(function() {
    ...
    // Use chrome.storage.local for extension storage (async)
    ...
    async function loadState() {
        try {
            const result = await storage.get([
                ...
                STORAGE_KEYS.CONTACTED_ADS,
                STORAGE_KEYS.KEYWORDS,
                STORAGE_KEYS.SETTINGS
            ]);

            // Hidden ads
            let hiddenAds = result[STORAGE_KEYS.HIDDEN_ADS] || [];
            // Migration from old format
            state.hiddenAds = hiddenAds.map(ad => {
            ...
            // Contacted ads
            state.contactedAds = result[STORAGE_KEYS.CONTACTED_ADS] || [];

            // Keywords
            state.keywords = result[STORAGE_KEYS.KEYWORDS] || [];
    ...
    function saveHiddenAds() {
        storage.set({ [STORAGE_KEYS.HIDDEN_ADS]: state.hiddenAds });
    }

    function saveContactedAds() {
        storage.set({ [STORAGE_KEYS.CONTACTED_ADS]: state.contactedAds });
    }

    function saveKeywords() {
        storage.set({ [STORAGE_KEYS.KEYWORDS]: state.keywords });
    }
    ...
    function extractAdData(container) {
        ...
        const titleEl = container.querySelector('.aditem-main--middle--title') ||
                        container.querySelector('a.ellipsis') ||
                        link;
        const priceEl = container.querySelector('.aditem-main--middle--price-shipping') ||
                        container.querySelector('.aditem-main--middle--price');
        const locationEl = container.querySelector('.aditem-main--top--left');
    ...
    function restoreAd(adId) {
        ...
        document.querySelectorAll(`.ka-placeholder[data-ad-id="${adId}"]`).forEach(placeholder => {
            ...
            if (nextSibling && (nextSibling.matches('li.ad-listitem') || nextSibling.matches('article.aditem'))) {
    ...
    function applyFilters() {
        const adContainers = document.querySelectorAll('li.ad-listitem, article.aditem');
        ...
        adContainers.forEach(container => {
            ...
            if (!restoredAdsWhitelist.has(id)) {
                ...
                const descEl = container.querySelector('.aditem-main--middle--description, [class*="description"]');
